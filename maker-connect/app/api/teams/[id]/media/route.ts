...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (body.type === 'cover') {
      if (!body.imageB64 || !body.contentType) return NextResponse.json({ error: 'imageB64 e contentType obrigatórios' }, { status: 400 });
      const { url } = await uploadCover(body.imageB64, body.contentType, `teams/${teamId}`);
      const t = await prisma.team.update({ where: { id: teamId }, data: { coverUrl: url }, select: { coverUrl: true } });
      return NextResponse.json({ data: t });
    }
