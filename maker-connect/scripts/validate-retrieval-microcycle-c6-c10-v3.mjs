...
const samples = [
  {
    ...
    curatedEvidence: [
      'ESP32 em Wi-Fi instavel: aplicar reconnect exponencial, persistir cache local e reenviar em lote na reconexao.',
      'Telemetria resiliente: manter buffer local e flush em lote apos reconnect para reduzir perda de dados.',
    ],
  },
  {
    id: 'C10',
    ...
    curatedEvidence: [
      'Pipeline LGPD: anonimiza de pii antes da inferencia, registro auditavel por etapa e saida final em json valido.',
      'Governanca: validar json no schema, armazenar trilha auditavel e bloquear vazamento de pii em logs.',
    ],
  },
];
...
function buildContext(sample) {
  const context = sample.curatedEvidence
...
function buildPrompt(sample) {
  ...
  const prompt = `Voce e o MakerBrain microcycle v3. Responda SOMENTE JSON valido em pt-BR.\n\nProjeto: ${sample.title}\nDominio: ${sample.domain}\nContexto: ${sample.input}\n\nKeywords obrigatorias: ${keywords}\n\nEvidencias curadas:\n${ctx.context}\n\nSchema obrigatorio:\n{"schemaVersion":"mc_extract_v2","technicalRequirements":[{"id":"TR-1","name":"string","detail":"string","priority":"medium"}],"suggestedBOM":[{"item":"string","quantity":"1","notes":"string"}],"suggestedCode":"","confidenceScore":0}\n\nRegras:\n1) technicalRequirements entre 3 e 5\n2) suggestedBOM entre 3 e 5\n3) priority em high|medium|low\n4) quantity como string\n5) suggestedCode sempre vazio\n6) Nao adicionar campos extras\n7) Cobrir todas as keywords obrigatorias no conjunto technicalRequirements + suggestedBOM\n8) Sem markdown e sem texto fora do JSON`;
...
function parseJsonFromResponse(text) {
  ...
  if (first >= 0 && last > first) {
    return tryParse(sanitized.slice(first, last + 1));
  }
...
function validateShape(output) {
  ...
  const reqValid = output.technicalRequirements.every(
    (req) => typeof req?.id === 'string' && typeof req?.name === 'string' && typeof req?.detail === 'string' && ['high', 'medium', 'low'].includes(String(req?.priority))
  );

  const bomValid = output.suggestedBOM.every(
    (item) => typeof item?.item === 'string' && typeof item?.quantity === 'string' && typeof item?.notes === 'string'
  );
...
async function run() {
  ...
  const report = {
    ...
    model: MODEL,
    endpoint: BASE_URL,
    retrievalExperiment: 'microcycle_c6_c10_balanced_v3',
