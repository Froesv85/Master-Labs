...
\textbf{Objetivos Especificos:}~(i)~Implementar pipeline funcional de IA com estagios de Extracao, RAG e Pos-processamento; (ii)~Orquestrar workflows via n8n com embeddings locais via Ollama/nomic-embed-text; (iii)~Construir base de conhecimento vetorial com dados reais de componentes eletronicos no Pinecone; (iv)~Modelar estrutura social com \textit{forks}, \textit{upvotes}, log de dificuldades e BOM via MySQL; (v)~Garantir conformidade LGPD com anonimizacao de PII; (vi)~Validar reducao do esforco documental por metricas objetivas de relevancia e latencia.
...
A qualidade do software e garantida por suite de 263 testes distribuidos em 39 suites (unitarios, de integracao e \textit{end-to-end}), cobrindo endpoints da API REST, pipeline de anonimizacao LGPD, fluxos de fork/upvote e ciclo assincrono de exportacao PDF. A execucao e integrada ao pipeline de CI/CD via GitHub Actions.
...
\begin{itemize}
  \item \textbf{Infraestrutura:} API REST em Next.js com Prisma ORM; banco relacional MySQL; servico de \textit{object storage} MinIO compativel com S3; orquestrador n8n com Ollama local; fila BullMQ/Redis como motor alternativo do pipeline de IA (\texttt{EXTRACTION\_ENGINE}); banco vetorial Pinecone.
  \item \textbf{IA:} pipeline RAG com embeddings nomic-embed-text; modelo generativo qwen2.5:7b-instruct; middleware LGPD com anonimizacao de PII; metricas de relevancia e latencia; \textit{endpoint} de metricas p50/p95.
  \item \textbf{Social:} feed de projetos com tags multiplas e visibilidade granular (publico / privado-dono / privado-equipe); fork com rastreamento de linhagem (\texttt{parentId}); upvote idempotente; log de dificuldades tecnicas; comunidades publicas/privadas; equipes com afiliacao mediante aprovacao e subsecao de competicoes; perfis de usuario; comentarios e reacoes.
...
Pre-proc. & Embeddings nomic-embed-text via Ollama; similaridade coseno; indexacao Pinecone & Vetores semanticos indexados \\
...
\hline
Frontend & Next.js 16 / React 19 & Interface maker: projetos, equipes, comunidades, pipeline de IA \\
Backend/ORM & Node.js + Prisma + MySQL & API REST; persistencia transacional; migracoes \\
Orquestrador & n8n + BullMQ + Ollama (local) & Coordenacao de agentes (motor duplo via \texttt{EXTRACTION\_ENGINE}); integracao de modelos LLM \\
Modelo LLM & qwen2.5:7b-instruct & Extracao de requisitos e geracao documental \\
Embeddings & nomic-embed-text via Ollama & Geracao de vetores semanticos para busca por similaridade \\
Vector DB & Pinecone & Base de conhecimento tecnico para RAG \\
Fila/Worker & BullMQ + Redis & Exportacao PDF assincrona e motor alternativo do pipeline de extracao, ambos com rastreamento de estado \\
...
CI/CD & GitHub Actions & Suite automatizada de 263 testes em cada push \\
\hline
\end{tblr}
\label{tab:arch}
\end{table}
...
Este trabalho contribui com: (i)~pipeline RAG funcional e avaliado empiricamente para o dominio IoT/maker, com base vetorial propria; (ii)~o mecanismo de fork com linhagem como instrumento de governanca tecnica em plataformas colaborativas; (iii)~modelo de conformidade LGPD-first integrado ao pipeline de IA desde a concepcao; (iv)~suite de 263 testes automatizados como referencia de qualidade para projetos academicos full-stack de alta complexidade.
...
Ollama + nomic-embed-text & Componente funcional do pipeline: geracao de embeddings semanticos para indexacao no Pinecone. Nao foi utilizado para escrita do artigo. \\
\hline
\end{tblr}
\end{table}
